...
import { getUser, logApi, planApi, formatDate } from "./api";
...
export default function RiwayatPenanaman() {
  ...
  const addRiwayat = async () => {
    setModalError("");

    if (!planId || succes === "" || fail === "") {
      setModalError("Silakan isi rencana, jumlah berhasil, dan jumlah gagal terlebih dahulu.");
      return;
    }
    ...
    if (selectedPlan && total > selectedPlan.count) {
      setModalError(
        `Berhasil + gagal (${total}) tidak boleh melebihi perkiraan jumlah tanaman pada rencana ini (${selectedPlan.count}).`
      );
      return;
    }
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          ...
          <div className="bg-white p-8 rounded-3xl w-[400px]">
            ...
            <div className="flex flex-col gap-4">
              ...
              >
                <option value="">Pilih rencana penanaman</option>
                {plans.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.plant?.name} - mulai {formatDate(p.started_at)}
                  </option>
                ))}
              </select>
              ...
              {planId && (
                <p className="text-[#888] text-xs -mt-2">
                  Berhasil + gagal tidak boleh melebihi{" "}
