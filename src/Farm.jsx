...
import { getUser, planApi, formatDate } from "./api";
...
export default function Farm() {
  ...
  const loadData = async (silent = false) => {
    ...
    try {
      ...
      if (!silent) setError(err.message || "Failed to load planting plan data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const removePlan = async (planId) => {
    if (!confirm("Delete this planting plan? Its schedule & history will also be removed.")) {
      return;
    }
    ...
    try {
      ...
      setError(err.message || "Failed to delete the planting plan.");
    } finally {
    ...
    <div className="min-h-screen bg-[#eef3f1]">
      ...
      <div className="flex gap-10">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-5 w-[65%]">
          {loading ? (
            <p className="text-center text-[#888] p-10">Loading calendar...</p>
          ) : (
            <Calendar plan={plans} onActivityChanged={() => loadData(true)} />
          )}
        </div>
                  ...
                  >
                    <div>
                      <p className="text-[#444] font-medium">
                        🌿 {item.plant?.name} ({item.area} m², {item.method}, ~{item.count} plants)
                      </p>
